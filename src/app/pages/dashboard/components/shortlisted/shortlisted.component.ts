...
export class ShortlistedComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (jobpostId) {
      ...
      this.apiService
        .getShortListedCandidates(jobpostId)
        .subscribe({
          next: (data) => {
  ...
  onFilterChange(filters: any) {
    ...
    //     form_data.field_of_study.lable !== this.filters.fieldOfStudy
    //   ) {
    //     return false;
    //   }
  ...
  onAdvanceFilterChange(filters: any) {
    ...
    this.filteredCandidates = this.candidates.filter((candidate) => {
      ...
      // Filter by jobposts (from resume_data.jobposts.name)
      const jobpostsMatch =
        this.advanceFilters.jobposts === '' ||
        resumeData?.projects?.some((projects) =>
          projects.name
            ?.toLowerCase()
            .includes(this.advanceFilters.jobposts.toLowerCase())
        );

      // Combine all filters
      return matchAny && skillsMatch && educationDegreeMatch && jobpostsMatch;
    });
  }
